...
def embed_cont_tensors(cont_tensors, embedding_dims):
    ...
    for i, (cont_tensor, embedding_dim) in (
        ...
        result.append(linear(cont_tensor, embedding_dim))
    return result
...
class Signal(object):
    ...
    def linear_output(self, input):
        ...
        cont_means = [
            linear(input, get_flatten_dim(cont_tensor))
            for cont_tensor in self.cont_tensors
        ]
        cat_logits = [
            linear(input, cat_vector.num_classes)
    ...
    def get_linear_output_loss(self, output):
        ...
        return [
            square_error(cont_tensor, cont_means)
            for cont_tensor in self.cont_tensors
        ] + [
    ...
    def feed_dict(self, cont_arrays, cat_arrays):
        result = dict([
            (pl, arr)
            for pl, arr in zip(self.cont_tensors, cont_arrays)
        ])
        for pl, arr in zip(self.cat_vectors, cat_arrays):
